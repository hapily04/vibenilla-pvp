...
public class VanillaPotionFeature implements PotionFeature, RegistrableFeature {
	...
	public void init(EventNode<EntityInstanceEvent> node) {
		...
		node.addListener(PlayerUseItemEvent.class, event -> {
            ...
            this.throwPotion(event.getPlayer(), event.getItemStack(), event.getHand(), false);
		});
		...
		node.addListener(PlayerUseItemEvent.class, event -> {
            ...
            this.throwPotion(event.getPlayer(), event.getItemStack(), event.getHand(), true);
		});
	}

	protected void throwPotion(Player player, ItemStack stack, PlayerHand hand, boolean lingering) {
		ThrownPotion thrownPotion = new ThrownPotion(player, this.effectFeature, lingering);
